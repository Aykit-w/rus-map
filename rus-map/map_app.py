...
class MapApp(tk.Tk):
    ...
    def on_click(self, event):
        ...
        print(f"Клик по координатам ({clicked_x},{clicked_y})")

        closest_city = None
        min_distance = float('inf')

        for city, data in CITIES_COORDINATES.items():
            cx, cy = data['coordinates']
            distance = ((cx - clicked_x)**2 + (cy - clicked_y)**2)**0.5
            if distance < min_distance:
                min_distance = distance
                closest_city = city

        if closest_city:
            city_data = CITIES_COORDINATES[closest_city]
            current_time = get_current_time(city_data['timezone'])

            info_window = tk.Toplevel(self)
            info_window.title(closest_city)
            info_window.geometry("300x200")

            coat_filename = city_data['coat']
            try:
                coat_of_arms = Image.open(coat_filename).resize((100, 100))
                coat_photo = ImageTk.PhotoImage(coat_of_arms)
                tk.Label(info_window, image=coat_photo).pack()
                info_window.image = coat_photo
            except Exception as e:
                tk.Label(info_window, text="Нет герба для этого города.").pack()

            tk.Label(info_window, text=f"Время в городе {closest_city}: {current_time}").pack()
